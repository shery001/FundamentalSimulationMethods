...
vector<double> do_rk4_step(vector<double> *curr_state, double delta_t){
// Do the fourth order Runge Kutta integration step for each element
// in the state vector
	int n_dim = curr_state -> size();
	vector<double> k1 = Rhs(curr_state);
// now add k1*\Delta t to the state vector
	vector<double> interm_state1;
	for(int i = 0; i < n_dim; ++i){
		interm_state1.push_back(curr_state->at(i) + k1.at(i)*delta_t*0.5);		
	}
// use the estimated value to compute f(y_est)	
	vector<double> k2 = Rhs(&interm_state1);
// k3 is calculatet exactly the same
	vector<double> interm_state2;
	for(int i = 0; i < n_dim; ++i){
		interm_state2.push_back(curr_state->at(i) + k2.at(i)*delta_t*0.5);		
	}
// use the estimated value to compute f(y_est)	
	vector<double> k3 = Rhs(&interm_state2);

// and the last is k4:        
	vector<double> interm_state3;
	for(int i = 0; i < n_dim; ++i){
		interm_state3.push_back(curr_state->at(i) + k3.at(i)*delta_t);		
	}
// use the estimated value to compute f(y_est)	
	vector<double> k4 = Rhs(&interm_state3);
        
// now calculate the new value for y:
	vector<double> result;
	for(int i = 0; i < n_dim; ++i){
		double this_addition = 1./6.*k1.at(i)+ 1./3.*k2.at(i) + 1./3.*k3.at(i) + 1./6.*k4.at(i);
		result.push_back(curr_state->at(i) + this_addition*delta_t);
	}
...
cout<<"time = "<<time_steps.at(i+1)<<endl;
	curr_state = do_rk4_step(&curr_state, delta_t);
